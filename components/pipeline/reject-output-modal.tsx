...
function SubmitButton({ hasReason }: { hasReason: boolean }) {
  ...
  return (
    <button
      type="submit"
      disabled={disabled}
      className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-[#0F0F0F] px-3.5 text-[12.5px] font-bold tracking-tight text-[#F4D93D] transition-all hover:-translate-y-px hover:bg-[#1A0F2A] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0"
      style={{
        fontFamily:
          'var(--font-inter-tight), var(--font-inter), sans-serif',
      }}
