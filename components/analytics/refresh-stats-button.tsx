 ...
 */
export function RefreshStatsButton({ workspaceId, lastFetchedAt }: RefreshStatsButtonProps) {
  const [pending, start] = useTransition()
  const [message, setMessage] = useState<string | null>(null)
  const [cooldownUntil, setCooldownUntil] = useState<number | null>(null)

  const inCooldown = cooldownUntil !== null && Date.now() < cooldownUntil
  const disabled = pending || inCooldown
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      ...
      >
        ...
        {pending ? 'Pulling…' : inCooldown ? 'Just refreshed' : 'Refresh now'}
      </button>
