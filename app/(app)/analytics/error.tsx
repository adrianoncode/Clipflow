...
export default function AnalyticsError({
  ...
  useEffect(() => {
    Sentry.captureException(error, { tags: { segment: 'analytics' } })
  }, [error])
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 p-8">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          >
            Couldn&rsquo;t load your numbers.
          </h1>
          <p className="mt-2 text-sm" style={{ color: '#7c7468' }}>
            {error.message ||
              'The analytics aggregator timed out or a Supabase query failed. Try again, or head back to the dashboard.'}
          </p>
          {error.digest ? (
            <p
